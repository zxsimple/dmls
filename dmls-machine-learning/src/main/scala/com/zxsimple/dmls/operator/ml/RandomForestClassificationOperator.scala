...
/**
  ...
  * 随机森林分类模型
  */
class RandomForestClassificationOperator extends OperatorTemplate {
  /**
    * 验证参数
    *
    * @param properties
  ...
  def train(hc: HiveContext, processId: Long, nodeVertexInstanceId: String, properties: Properties, trainInputData: DataFrame): RandomForestModel = {
    ...
    model = RandomForest.trainClassifier(trainData, numClasses, categoricalFeaturesInfo,
      numTrees, "auto", impurity, maxDepth, maxBins)
