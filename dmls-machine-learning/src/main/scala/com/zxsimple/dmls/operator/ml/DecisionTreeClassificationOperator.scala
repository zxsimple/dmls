...
/**
  ...
  * 决策树分类模型
  */
class DecisionTreeClassificationOperator extends OperatorTemplate {
  /**
    * 验证参数
    *
    * @param properties
  ...
  override def runTask(hc: HiveContext, currentNode: NodeVertex, inputPortOneData: DataFrame, inputPortTwoData: DataFrame): Unit = {
    ...
    //首先进入训练
    val decisionTreeClassificationModel = train(hc, processId, nodeVertexInstanceId, properties, inputPortOneData)

    //如果第二个参数不为空，则进入应用
    if (inputPortTwoData != null) {
      val result = predict(hc, processId, nodeVertexInstanceId: String, decisionTreeClassificationModel, properties, inputPortTwoData)
  ...
  def train(hc: HiveContext, processId: Long, nodeVertexInstanceId: String, properties: Properties, trainInputData: DataFrame): DecisionTreeModel = {
    ...
    val impurity = properties.getProperty("impurity")
    ...
    model = DecisionTree.trainClassifier(trainData, numClasses, categoricalFeaturesInfo,
