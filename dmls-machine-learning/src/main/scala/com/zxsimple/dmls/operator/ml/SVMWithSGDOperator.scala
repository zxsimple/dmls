...
/**
  ...
  * 支持向量机分类模型
  */
class SVMWithSGDOperator extends OperatorTemplate {
  /**
    * 验证参数
    *
    * @param properties
  ...
  /**
    ...
    * @return
    */
  def train(hc: HiveContext, processId: Long, nodeVertexInstanceId: String, properties: Properties, trainInputData: DataFrame): SVMModel = {
    ...
    val stepSize = properties.getProperty("stepSize").toDouble
    val regParam = properties.getProperty("regParam").toDouble
    val miniBatchFraction = properties.getProperty("miniBatchFraction").toDouble
    ...
    //模型训练
    model = SVMWithSGD.train(trainData, numIterations, stepSize, regParam, miniBatchFraction)
  ...
  /**
    * 应用模型
    */
  def predict(hc: HiveContext, processId: Long, nodeVertexInstanceId: String, model: SVMModel, properties: Properties, predictInputData: DataFrame) = {
